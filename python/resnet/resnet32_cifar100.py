...
def sample_blocks(num_layers, num_approx):
    ...
    perms = []
    for _ in range(1000):
        perms.append(sorted(random.sample(list(range(1,num_layers-2)), num_approx)))
    # Remove duplicates
    perms.sort()
...
class Cifar100Model(Trainable):
    ...
    def _build_model(self):
        ...
        opt = tf.keras.optimizers.SGD(lr=self.lr,
                                   momentum=self.mom,
                                   clipvalue=2.0)
        model.compile(
...
if __name__ == "__main__":
    ...
    train_spec = {
        ...
        "config": {
            ...
            "approx": sample_from(lambda _: random.choice(sample_blocks(32, args.approx))),
            "swap_period": 100,
