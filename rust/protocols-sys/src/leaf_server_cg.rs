...
pub trait LServerCG {
    ...
    fn l_preprocess(&mut self, kernel: &Kernel<u64>,linear_share: &Output<u64>) ->(Vec<i8>,Vec<i8>);
...
impl<'a> SealLeafServerCG<'a> {
    ...
    pub fn l_preprocess(&mut self,kernel: &Kernel<u64>, linear_share: &Output<u64>) ->(Vec<i8>,Vec<i8>) {
        match self {
            Self::Conv2D(s) => s.l_preprocess(kernel,linear_share),
            Self::FullyConnected(s) => s.l_preprocess(kernel,linear_share),
        }
    }
...
impl<'a> LServerCG for Conv2D<'a> {
    ...
    fn l_preprocess(&mut self, kernel: &Kernel<u64>, linear_share: &Output<u64>) ->(Vec<i8>,Vec<i8>){
        ...
        let shares = unsafe {
            server_bc_l_conv_preprocess(self.lsmphe, &self.data ,kernel_vec.as_ptr(),linear_vec.as_ptr())
        };
        self.shares = Some(shares);
        let weight_ct_vec = unsafe {
            std::slice::from_raw_parts(shares.weight_ct.inner, shares.weight_ct.size as usize)
...
impl<'a> LServerCG for FullyConnected<'a> {
    ...
    fn l_preprocess(&mut self, kernel: &Kernel<u64>,linear_share: &Output<u64>)  ->(Vec<i8>,Vec<i8>){
        ...
        linear = linear_share
            ...
            // println!{""}
            let shares = unsafe {
                server_bc_l_fc_preprocess(self.lsmphe, &self.data ,kernel_vec.as_ptr(),linear)
            };
            self.shares = Some(shares);
            let weight_ct_vec = unsafe {
                std::slice::from_raw_parts(shares.weight_ct.inner, shares.weight_ct.size as usize)
