...
impl KeyShare {
    ...
    pub fn leaf_mphe_r2(&mut self, mut lsmphe:  LeafServerMPHE, mut r1_share: Vec<std::os::raw::c_char>)  -> Vec<std::os::raw::c_char>{
        ...
        unsafe{server_mphe_r2(mphe, &mut self.0,ser_r1_ct)};
        unsafe {
            from_raw_parts(self.0.inner, self.0.size as usize).to_vec()
        }
        
    }
    pub fn root_mphe_receive_r2(&mut self,mut rsmphe: RootServerMPHE,mut keys_vecs_a: Vec<std::os::raw::c_char>,mut keys_vecs_b: Vec<std::os::raw::c_char>,mut keys_vecs_c: Vec<std::os::raw::c_char>) 
    {
        ...
        unsafe{server_mphe_aggregation_r2(mphe,ser_a_ct,ser_b_ct,ser_c_ct)};
