...
vector<Ciphertext> mphe_preprocess_matrix(const u64* const* matrix, const Metadata& data, BatchEncoder& batch_encoder, Encryptor& encryptor) {
    ...
    // Encode matrix vectors
    vector<Ciphertext> enc_mat(data.inp_ct);
    for (int ct = 0; ct < data.inp_ct; ct++) {
        Plaintext tmp;
        batch_encoder.encode(mat_diag[ct], tmp);
        encryptor.encrypt(tmp,enc_mat[ct]);
    }
    return enc_mat;
}
...
Ciphertext mphe_fc_preprocess_noise(const Metadata &data, BatchEncoder& batch_encoder, Encryptor& encryptor, const uint64_t* secret_share) {
    ...
    // Encode the noise vector
    Plaintext encoded_noise;
    Ciphertext encrypted_noise;
    batch_encoder.encode(noise, encoded_noise);
    encryptor.encrypt(encoded_noise,encrypted_noise);
...
Ciphertext mphe_fc_online(Ciphertext& ct, vector<Ciphertext>& enc_mat,
        const Metadata& data, Evaluator& evaluator, GaloisKeys& gal_keys,
        RelinKeys& relin_keys, Ciphertext& zero) {
    Ciphertext result = zero;
    // For each matrix ciphertext, rotate the input vector once and multiply +
