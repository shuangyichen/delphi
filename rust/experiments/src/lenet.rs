...
pub fn construct_lenet_gateway<R: RngCore + CryptoRng>(
    // nn: &mut NeuralNetwork<TenBitAS, TenBitExpFP>,
    vs: Option<&tch::nn::Path>,
    split_layer: usize,
    batch_size: usize,
...
pub fn construct_lenet_remote<R: RngCore + CryptoRng>(
    // nn: &mut NeuralNetwork<TenBitAS, TenBitExpFP>,
    vs: Option<&tch::nn::Path>,
    split_layer: usize,
    batch_size: usize,
    ...
    let mut nn = construct_lenet(None,batch_size,0,rng);
    let layers = nn.layers.len();
    // println!("Full network {} layers", layers);
    let out_layers = layers-split_layer;
...
pub fn construct_lenet<R: RngCore + CryptoRng>(
    vs: Option<&tch::nn::Path>,
    batch_size: usize,
    num_poly: usize,
    rng: &mut R,
