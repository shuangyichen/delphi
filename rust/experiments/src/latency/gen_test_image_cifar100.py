...
def generate_image(image_path):
    ...
    image = x_test[4]
    classification = y_test[4]
    print(classification)
    _, _, chans = image.shape
    rust_tensor = np.array([[image[:, :, c] for c in range(chans)]])
    np.save(os.path.join(image_path, f"image_100.npy"), rust_tensor.flatten().astype(np.float64))
    np.save(path.join(image_path, f"class_100.npy"), np.array(classification).flatten().astype(np.int64))
